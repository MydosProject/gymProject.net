...
public class ServicePackageCatalogService(ApplicationDbContext dbContext)
{
    ...
    public async Task<IReadOnlyList<ServicePackageCardViewModel>> LoadAsync(ServicePackageCategory category)
    {
        var packages = await dbContext.ServicePackages.AsNoTracking()
            ...
            .ToListAsync();
        var regularPtReference = packages.Where(x => x.Slug == "pt-flex")
        ...
        {
            var featureTexts = package.Features.OrderBy(x => x.DisplayOrder).Select(x => x.Text).ToList();
            return new ServicePackageCardViewModel
            {
                ...
                Features = featureTexts.Where(x => !IsCommercialHighlight(x)).ToList(),
                Highlights = featureTexts.Where(IsCommercialHighlight).Concat(BenefitLabels(package)).ToList(),
                Variants = package.Variants.Where(x => x.IsActive &&
                        (category != ServicePackageCategory.KidsClub || x.KidsClassCreditCount == 8))
                    .OrderBy(x => x.DisplayOrder)
                    .Select(x => Present(x, package.Variants, PtReference(package, regularPtReference))).ToList()
            };
        }).ToList();
    }
        ...
        IEnumerable<ServicePackageVariant>? siblings = null, ServicePackageVariant? ptReference = null)
    {
        ...
        if (!variant.PriceOnRequest)
        {
            ...
            {
                unitPrice = $"{Money(variant.TotalPrice / TotalLessonCount(variant))} / ders";
                var reference = variant.LessonsRenewMonthly && variant.DurationMonths is > 1
                    ? siblings?.Where(x => x.IsActive && !x.PriceOnRequest &&
                        x.BillingType == ServicePackageBillingType.OneTime && x.DurationMonths == 1 &&
                        SameRights(x, variant)).OrderBy(x => x.TotalPrice).FirstOrDefault()
                    : siblings?.Where(x => x.IsActive && !x.PriceOnRequest &&
                        ...
                        x.LessonsRenewMonthly == variant.LessonsRenewMonthly &&
                        TotalLessonCount(x) > 0 && TotalLessonCount(x) < TotalLessonCount(variant))
                        .OrderBy(x => TotalLessonCount(x)).FirstOrDefault();
                if (reference is not null)
                {
                    if (variant.LessonsRenewMonthly && variant.DurationMonths is > 1 && SameRights(reference, variant))
                    {
                        referenceTotal = reference.TotalPrice * variant.DurationMonths.Value;
                        comparison = $"{reference.Name} paketinin {variant.DurationMonths} aylık bedeline göre";
                    }
                    else
                    {
                        referenceTotal = reference.TotalPrice / TotalLessonCount(reference) * TotalLessonCount(variant);
        ...
        return new ServicePackageVariantCardViewModel
        {
            ...
            BillingNote = monthly
                ? "Üyelik bedeli her ay karttan otomatik tahsil edilecek şekilde planlanır. Başvuru tek başına ödeme oluşturmaz; kart talimatı ve tahsilat koşulları üyelik aktivasyonunda ayrıca onaylanır."
                : null,
            CommitmentNote = variant.BonusMonths > 0 && variant.DurationMonths is > 0
                ? $"{variant.DurationMonths} aylık paketin {variant.BonusMonths} ayı hediye. Hediye ay dahil toplam kullanım süresi {variant.DurationMonths} ay."
    ...
    private static bool IsCommercialHighlight(string text) =>
        text.Contains("indirim", StringComparison.OrdinalIgnoreCase) ||
        text.Contains("hediye", StringComparison.OrdinalIgnoreCase) ||
        text.Contains("ücretsiz", StringComparison.OrdinalIgnoreCase);
    ...
    private static int TotalLessonCount(ServicePackageVariant x) => LessonCount(x) *
        (x.LessonsRenewMonthly && x.DurationMonths is > 1 ? x.DurationMonths.Value : 1);
    ...
    private static ServicePackageVariant? PtReference(ServicePackage package, ServicePackageVariant? regularReference) =>
        package.Slug == "duet-pt"
            ? package.Variants.Where(IsPricedPt).OrderBy(x => x.PersonalTrainingSessionCount).FirstOrDefault()
            : regularReference;
